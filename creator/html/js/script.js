...
$(document).ready(function () {
	GCS.Subscribe('message', function (event) {
		if (event.data.action == 'setVisible') {
			...
			$('#' + event.data.tab).show()
			$.post('https://creator/setCurrentTab', JSON.stringify({
				tab: event.data.tab,
			}));
		}
...
function setView(event, view) {
	...
	if (!wasActive) {
		$.post('https://creator/setCameraView', JSON.stringify({
			view: view
		}));
		$.post('https://creator/playSound', JSON.stringify({
...
$('#cameracontrol').on('mousemove', function (event) {
	if (moving == true) {
		...
		lastOffsetX = offsetX;
		lastOffsetY = offsetY;
		$.post('https://creator/updateCameraRotation', JSON.stringify({
			x: offsetX,
			y: offsetY,
		}));
	}
...
$('#cameracontrol').on('wheel', function (event) {
	...
	$.post('https://creator/updateCameraZoom', JSON.stringify({
		zoom: zoom,
	}));
});
...
function loadTabContent(tabName, charData, clothesData, identityData) {
	$.get('pages/' + tabName + '.html', function (data) {
		...
		else if (identity && (tabName == 'identity')) {
...
function loadOptionalIdentity(element, identityData) {
	...
	$.get('pages/optional/identity.html', function (data) {
		pdata.html(data);

		// set the limits (config)
		pdata.find('.name').each(function () {
			$(this).prop('placeholder', 'max. ' + 16 + ' characters');
		});
		let sliderheight = pdata.find('input[type="range"]').first();
		sliderheight.prop('min', 48);
		sliderheight.prop('max', 300);
...
function closeWindow(save) {
	if ((identity) && save && editedTabs.includes('identity')) {
		...
		if (dateCheck == "Invalid Date")
			...
			$.post('https://identity/register', JSON.stringify({
	...
	$.post('https://creator/close', JSON.stringify({
		save: save
	}));
}
...
$('.panelbottom button').on('click', function (evt) {
	evt.preventDefault();
	$.post('https://creator/playSound', JSON.stringify({ sound: 'panelbuttonclick' }));
});
...
function updateGender(value) {
	$.post('https://creator/updateGender', JSON.stringify({
		value: value,
	}));
}

function updateHeadBlend(key, value) {
	$.post('https://creator/updateHeadBlend', JSON.stringify({
		key: key,
		value: value,
	}));
}
...
function updateFaceFeature(key, value, index) {
	$.post('https://creator/updateFaceFeature', JSON.stringify({
		key: key,
		value: value,
		index: index,
	}));
...
function updateEyeColor(value) {
	$.post('https://creator/updateEyeColor', JSON.stringify({
		value: value,
	}));
}

function updateHairColor(key, value, highlight) {
	$.post('https://creator/updateHairColor', JSON.stringify({
		key: key,
		value: value,
		highlight: highlight,
	}));
...
function updateHeadOverlay(key, keyPaired, value, index, isOpacity) {
	$.post('https://creator/updateHeadOverlay', JSON.stringify({
		key: key,
...
function updateHeadOverlayExtra(key, keyPaired, value, index, keyExtra, valueExtra, indexExtra, isOpacity) {
	$.post('https://creator/updateHeadOverlayExtra', JSON.stringify({
		key: key,
		keyPaired: keyPaired,
		value: value,
		index: index,
...
function updateOverlayColor(key, value, index, colortype) {
	$.post('https://creator/updateOverlayColor', JSON.stringify({
		key: key,
		value: value,
		index: index,
		colortype: colortype,
...
function updateComponent(drawable, dvalue, texture, tvalue, index) {
	$.post('https://creator/updateComponent', JSON.stringify({
		drawable: drawable,
		dvalue: dvalue,
		texture: texture,
		tvalue: tvalue,
...
function updateApparelComponent(drwkey, drwval, texkey, texval, cmpid) {
	$.post('https://creator/updateApparelComponent', JSON.stringify({
		drwkey: drwkey,
		drwval: drwval,
		texkey: texkey,
		texval: texval,
...
function updateApparelProp(drwkey, drwval, texkey, texval, propid) {
	$.post('https://creator/updateApparelProp', JSON.stringify({
		drwkey: drwkey,
		drwval: drwval,
		texkey: texkey,
		texval: texval,
...
$(document).on('click', 'input:radio[name=sex]', function (evt) {
	if (radioChecked == false) {
		let popupData = {
			title: 'confirmation',
			message: 'Changer le sexe de votre personnage réinitialisera toutes les personnalisations actuelles. Êtes-vous sûr de vouloir faire ça ?'
		};
...
$(document).on('change', 'select.facepaintoverlay', function (evt) {
	...
	if (prevtype != type) {
		$.post('https://creator/syncFacepaintOpacity', JSON.stringify({
			prevtype: prevtype,
			currenttype: type,
		}));
		...
		$.post('https://creator/clearMakeup', JSON.stringify({
			clearopacity: false,
			clearblusher: true,
		}));
	}
...
$(document).on('change', 'select.makeuptype', function (evt) {
	...
	$.post('https://creator/updateMakeupType', JSON.stringify({ type: value }));
});
...
function refreshIdentityData(element, data) {
	if (data) {
		...
		// these are defined in identity.html <script> tag.
		checkName('firstname');
		checkName('lastname');
		checkDOB();
