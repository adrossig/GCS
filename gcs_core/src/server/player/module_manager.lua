...
function PlayerManager:PlayerConnect(source, name, setKickReason, deferrals)
	...
	if not licenseIdentifier then
		deferrals.done(SERVER_Config.Deferrals.NotConnectedToSteam)
		CancelEvent()
		return
	else
		if not self.whitelist_enabled or IsPlayerAceAllowed(source, "whitelist") then
			deferrals.update(SERVER_Config.Deferrals.Connecting)
		else
			deferrals.done(SERVER_Config.Deferrals.NotWhitelisted)
			CancelEvent()
			return
		end
	end
	...
	-- Check if the server is full or if there's a queue
		if self:GetNumPlayersConnected() >= self.max_players or #self.queue > 0 then
			...
			while self:GetNumPlayersConnected() >= self.max_players or self:GetPositionInQueue(source) > 1 do
				...
				if not GetPlayerEndpoint(source) then
					...
					deferrals.done(SERVER_Config.Deferrals.ConnectionCancelled)
					CancelEvent()
					return
				end

				deferrals.update(string.format(SERVER_Config.Deferrals.WaitingInQueue, 
		...
		deferrals.update(SERVER_Config.Deferrals.Connected)
		deferrals.done()
		self.num_players_connected = self.num_players_connected + 1
	end)
...
function PlayerManager:GetLoadingIcons(time)
	local str = ""
	for i = 1, time % 6 do
		str = str .. SERVER_Config.Deferrals.LoadingIcon
	end
	return str
end
...
function PlayerManager:GetPositionInQueue(source)
	...
	for i = 1, #self.queue do
		if self.queue[i] == source then return i end
	end
	return '???'
end
