...
## <a name='loop'>Loop</a>
  ...
  - For an optimisation issue we will always use an # in foor loops for tables and lists.
	...
	- Conclusion of for-loops in lua:
	- Do not use pairs() or ipairs() in critical code! 
...
## <a name='addingTable'>Adding Table Items</a>

  - The proper optimised way to insert a value in a table or in a lists.
	...
	- Conclusion: Do not use table.insert!
	- Try to save the table-size somewhere and use a[count + i] = x !
...
## <a name='file-structrure'>File Structure</a>
  ...
  - For each project, the folders are:
  - doc: Any notes and other documentation are stored here.
  - include: All dependencies which are themselves lua files.
  - data: All data dependencies which are themselves json files.
  - html: All web dependencies which are themselves lua files.
  - lib: Any dependencies which are compiled binaries such as .dll and .so files.
  - src: Any source files which are local to this project.
  - test: Anything related to unit tests, if applicable.
	...
	```
		...
		│   └── Web-1.52
		│       └── web.lua
		├── libs                          # Binary dependencies
		│   └── Lrexlib-PCRE-2.9.0-2
		│       ├── pcre.dll
		│       └── lrex_pcre53.dll
