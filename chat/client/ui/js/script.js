$(document).ready(function() 
{
	...
	function OnKeyUp(keycode, e)
	{
		if (keycode == send_key && typing && !transitioning)
		{
			...
			{
				GCS.CallEvent('chat/submit_message', {msg: msg, channel: current_channel})
			...
			GCS.CallEvent(`chat/input_state`, {state: false});
			$('html').css('pointer-events', 'none');
			$('html').blur();
			can_send = false;
			setTimeout(() => 
		...
		else if (keycode == type_key && !typing && !transitioning && can_open)
		{
			...
			$("#input-area").val("");
			$("#input-area").select();
			GCS.CallEvent(`chat/input_state`, {state: true});
		}
	...
	function ToggleOpen()
	{
		...
		GCS.CallEvent('chat/toggle_visible', {visible: open});
	}
	...
	GCS.Subscribe('chat/toggle_enabled', function(args)
	{
		can_open = args.can_open;
	})

	GCS.Subscribe('chat/init_config', function(args)
	{
		InitConfig(args);
	})

	GCS.Subscribe('chat/add_message', function(args)
	{
		AddMessage(args);
	})

	GCS.Subscribe('chat/clear_chat', function()
	{
		ClearChat();
	})

	GCS.Subscribe('chat/toggle_open', function()
	{
		ToggleOpen();
	})

	GCS.Subscribe('chat/store_name', function(args)
	{
		my_name = args.name;
	})

	GCS.Subscribe('KeyUp', function(args)
	{
		return OnKeyUp(args.key, args.event);
	})

	GCS.Subscribe('KeyDown', function(args)
	{
		return PreventScrolling(args.key, args.event)
	})

	GCS.Subscribe('KeyPress', function(args)
	{
		return PreventScrolling(args.key, args.event)
	})
			
	GCS.Subscribe('chat/remove_player', function(args)
	{
		const name = args.name;
		if (players.indexOf(name) > -1) {players.splice(players.indexOf(name), 1);}
	})

	GCS.Subscribe('chat/start_typing', function()
	{
		OnKeyUp(type_key);
	})

	GCS.Subscribe('chat/add_player', function(args)
	{
